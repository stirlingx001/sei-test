...
library MsgDataCodec {
    function decode(uint64 initial_pos, bytes memory buf, uint64 len) internal pure returns (bool, uint64, MsgData memory) {
        // Message instance
        MsgData memory instance;
    ...
    function decode_field(uint64 initial_pos, bytes memory buf, uint64 len, uint64 field_number, MsgData memory instance) internal pure returns (bool, uint64) {
        uint64 pos = initial_pos;
    ...
    // MsgData.msg_type
    function decode_1(uint64 pos, bytes memory buf, MsgData memory instance) internal pure returns (bool, uint64) {
        bool success;
    ...
    // MsgData.data
    function decode_2(uint64 pos, bytes memory buf, MsgData memory instance) internal pure returns (bool, uint64) {
        bool success;
...
library TxMsgDataCodec {
    function decode(uint64 initial_pos, bytes memory buf, uint64 len) internal pure returns (bool, uint64, TxMsgData memory) {
        // Message instance
        TxMsgData memory instance;
        // Previous field number
        uint64 previous_field_number = 0;
        // Current position in the buffer
        uint64 pos = initial_pos;
    ...
    function decode_field(uint64 initial_pos, bytes memory buf, uint64 len, uint64 field_number, TxMsgData memory instance) internal pure returns (bool, uint64) {
        uint64 pos = initial_pos;
    ...
    // TxMsgData.data
    function decode_1(uint64 pos, bytes memory buf, TxMsgData memory instance) internal pure returns (bool, uint64) {
        bool success;
        ...
        for (uint64 i = 0; i < cnt; i++) {
            ...
            MsgData memory nestedInstance;
            (success, pos, nestedInstance) = MsgDataCodec.decode(pos, buf, len);
            if (!success) {
                return (false, pos);
            }

            instance.data[i] = nestedInstance;
    ...
    // TxMsgData.msg_responses
    function decode_2(uint64 pos, bytes memory buf, TxMsgData memory instance) internal pure returns (bool, uint64) {
        bool success;
        ...
        // Allocated memory
        instance.msg_responses = new Any[](cnt);
        ...
        for (uint64 i = 0; i < cnt; i++) {
            ...
            Any memory nestedInstance;
            (success, pos, nestedInstance) = AnyCodec.decode(pos, buf, len);
            if (!success) {
                return (false, pos);
            }

            instance.msg_responses[i] = nestedInstance;
